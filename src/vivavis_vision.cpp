...
void VivavisVision::cloudCallback(const sensor_msgs::PointCloud2ConstPtr &input)
{
    ...
    pass_x.setFilterLimits(nearClipDistance, farClipDistance);
    pass_x.setInputCloud(cloud);
    pass_x.setKeepOrganized(true);
    pass_x.filter(*cld_tmp_z);
    try
    {
...
//                                         MAIN
//------------------------------------------------------------------------------------------------
int youbot_vision_main(int argc, char **argv)
{
    ros::init(argc, argv, "youbot_vision_node");
    ros::NodeHandle nh;
    VivavisVision yb_vision(nh);
...
{
    return youbot_vision_main(argc, argv);
}